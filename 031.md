...
К сожалению, команда `sign-req` не автоматизируема, что связано с невозможностью осуществления автоматического ввода ответов на вопросы утилиты
```shell
cd ../vm
vagrant ssh server
```
...
● openvpn@server.service - OpenVPN Robust And Highly Flexible Tunneling Application On server
 ...
 Main PID: 4180 (openvpn)
   ...
   CGroup: /system.slice/system-openvpn.slice/openvpn@server.service
           └─4180 /usr/sbin/openvpn --cd /etc/openvpn/ --config server.conf
```
...
Sat Sep 18 12:07:08 2021 Initialization Sequence Completed
```
...
К сожалению, команда `sign-req` не автоматизируема
...
```shell
cd /home/vagrant/common/temp/easyrsa/ && ./easyrsa sign-req client client # <--- вот тут вводим не автоматизируемые ответы
```
