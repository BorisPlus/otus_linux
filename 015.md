...
* добавить в `/etc/security/time.conf` записи вида
        ```
        # <пользователи группы admin> - это перечисление вида `admin001&admin002&admin003`
        #
        login;*;!<пользователи группы admin>;AlSaSu  # дает разрешение использования по времени
                                                     # `AlSaSu` (всегда, кроме субботы и воскресенья) 
                                                     # службы `login`
                                                     # для пользователей не admin-группы
        #
        login;*;!<пользователи группы admin>;!Al     # не дает каких-либо иных разрешений 
